...
namespace  App\Http\Controllers; 
 ...
 $penanamans =  $query->latest()->paginate(( int )  $perPage )->withQueryString(); 
         ...
         $penanamans->getCollection()->transform(function ($item) use ($role) {
            $item->status_evaluasi = $this->resolveStatusEvaluasi($item, $role);
            return $item;
        }); 
 ...
 $status =  $this->resolveStatusEvaluasi( $penanaman, $role ); 
 ...
 return back ()->with( 'error' , $e->getMessage()); 
    ...
    private function resolveStatusEvaluasi($penanaman, $role = 'guru'): string
    { 
 ...
 return 'Data Awal Kurang' ; 
        ...
        return $role === 'siswa' ? 'Menunggu Evaluasi Guru' : 'Siap Evaluasi';
    } 
 
    public function show($id)
    {
        $penanaman = \App\Models\Penanaman::with(['siswa', 'jenisTanaman', 'pemeliharaans', 'panen', 'evaluasi.evaluator'])->findOrFail($id);
        $role = strtolower(auth()->user()->role);
        if ($role === 'siswa' && $penanaman->siswa_id !== auth()->id()) {
            abort(403, 'Unauthorized access to evaluation data.');
        }
        
        if (view()->exists($role . '.evaluasi.show')) {
            return view($role . '.evaluasi.show', compact('penanaman'));
        }
        
        return redirect()->route($role . '.evaluasi.index')->with('success', 'Lihat detail evaluasi untuk: ' . $penanaman->jenis_tanaman);
    }
