...
classDecisionTreeService
{
publicfunctionevaluate($penanaman): array
    {
        $penanaman->load([
'jenisTanaman',
'pemeliharaans',
...
if (!$panen) {
thrownew \Exception('Data panen belum tersedia.');
        }

if (!$pemeliharaans|| $pemeliharaans->count() ===0) {
thrownew \Exception('Data pemeliharaan belum tersedia.');
        }

$jumlahBibit= (int) $penanaman->jml_bibit;

if ($jumlahBibit0) {
thrownew \Exception('Jumlah bibit tidak valid.');
        }

$totalTanamanPanen= (int) $panen->tanaman_hidup + (int) $panen->tanaman_mati;

if ($totalTanamanPanen>$jumlahBibit) {
thrownew \Exception('Total tanaman hidup dan mati melebihi jumlah bibit awal.');
        }
...
if ($targetPanen0) {
thrownew \Exception('Target panen belum tersedia.');
        }
...
'aturan' => [
'Gagal jika persentase hidup ,
'Gagal jika persentase hasil ,
...
privatefunctiongetTargetDanHasil($penanaman, $panen): array
    {
...
privatefunctiongetHamaTerberat($pemeliharaans): string
    {
$level= [
'Tidak Ada' => 0,
'Ringan' => 1,
...
privatefunctiongetRisikoTerburuk($pemeliharaans): string
    {
$level= [
'Aman' => 0,
'Perlu Pantauan' => 1,
...
privatefunctionfallbackRisiko($pm): string
    {
...
privatefunctionnilaiHama(string$hama): int
    {
returnmatch ($hama) {
'Tidak Ada' => 100,
'Ringan' => 85,
...
privatefunctionnilaiPertumbuhan(string$status): int
    {
returnmatch ($status) {
'Aman' => 100,
'Perlu Pantauan' => 70,
...
privatefunctiongetFaktorUtama($hidup, $hasil, $hama, $risiko): array
    {
$faktor= [];
...
privatefunctiongetRekomendasi(string$hasil): array
    {
returnmatch ($hasil) {
'Berhasil' => [
