...
classEvaluasiControllerextends Controller
{
...
returnview($viewName, compact('penanamans', 'stats'));
    }

publicfunctionproses(Request$request, DecisionTreeService$service)
    {
        $request->validate([
...
returnback()->with('error', 'Data belum siap dievaluasi. Status saat ini: '.$status);
        }

if ($penanaman->evaluasi) {
returnback()->with('error', 'Data ini sudah pernah dievaluasi.');
        }
...
returnback()->with('error', $e->getMessage());
        }
    }

privatefunctionresolveStatusEvaluasi($penanaman): string
    {
